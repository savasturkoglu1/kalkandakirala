    ...
    <div class="container pr">
        ...
        <div class="col-md-12">   
            ...
            <div class="tabs"  v-if="!vilId">
                    <ul>
                        <li :class="show==1?'is-active':''" @click="showOne"><a>Tarih Aralığı</a></li>
                        <li  :class="show==2?'is-active':''" @click="()=>{ this.show = 2 }"><a>Esnek Tatil Periyodu</a></li>                        
                    </ul>
            </div>
             <nav class="level" v-if="!inline && !mobile " >
               
               <div class="buttons inf-buttons" v-if="vilId">
                   <button class="button is-small" v-if="vilId">
                       <span class="ui-state ui-red" ></span> Kiralanmış
                   </button>
                   <button class="button is-small" v-if="vilId">
                        <span  class="ui-state ui-yellow" ></span> Opsiyonlanmış
                   </button>
            ...
            <div class="" v-if="show==2">
             ...
             <nav class="level inf-buttons" v-if="inline">
               
               <div class="buttons">
                   <button class="button" v-if="vilId">
                       <span class="ui-state ui-red" ></span> Kiralanmış
                   </button>
                   <button class="button" v-if="vilId">
                        <span  class="ui-state ui-yellow" ></span> Opsiyonlanmış
              ...
              <div class="sec-buton-cont" v-if="!mobile"> 
                    <button :class="esnek?'button button-small is-primary clear-buton':'button button-small clear-buton'" style="float: left;" v-if="!vilId" @click="()=>{this.esnek = ! this.esnek}"> +-3 gün </button>   
                     <button class="button button-small is-clear" v-if="date2 && !inline" @click="clean">Temizle</button>  
                         <button class="button button-small is-clear" v-if="!inline" type="button" @click="clean">Kapat</button>                    
                     <button class="button button-small sec-buton bg-primary-normal-gradient text-w" v-if="!inline"  v-on:click="sendDates">Seç</button>
             </div>
        </div>
    </div>
</template>
...
export default {   
    ...
    mounted(){
        ...
        if(window.innerWidth<1100){this.t_size=[1,1]; }
          if(process.client){
            if(!this.vilId){
      ...
      eDates(a,e){
        ...
        sendDates(){
              ...
              if(!this.vilId){
                    ...
                    this.$emit('cBack', this.date1, this.date2);
              }
        ...
        getPeriods(){
            this.spn = true;
             this.$axios.get('/periyotlar/'+this.vilId).then(resp => {
                 // console.log('tarihler', resp.data)
