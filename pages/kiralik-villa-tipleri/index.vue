        ...
        <div class="columns is-multiline">
            <!-- <div class="column is-4  un-col " v-for="(item, i) in data" :key="i">
                <div class="un-wrap "  :style="{'animation-delay': 0.1*i+'s'}">
                    <img class="un-img" :src="item.kat_kapak" />
                    <div class="un-desc">
                        <h3> {{item.kat_adi}} </h3>
                        
                        <span> {{item.count}} Villa </span>
                         <div class="mt-20">
                              <nuxt-link class="button is-primary is-rounded" :to="$store.state.trUrls.kats+item.kat_url" :title="item.kat_adi">İncele</nuxt-link>
                         </div>
                    </div>
                </div>
            </div> -->
...
export default {
     head() {
            return {
                title: this.katSeo.seo.seo_baslik,
               meta: [
                 { hid: 'description', name: 'description', content: this.katSeo.seo.seo_aciklama },
               
              
                { property:"og:image" , content:this.katSeo.kapak },
                ...
                { property:"og:title" , content:this.katSeo.seo.seo_baslik },
                { property:"og:description" , content:this.katSeo.seo.seo_aciklama },
                ...
                { name:"twitter:title" , content:this.katSeo.seo.seo_baslik },
                { name:"twitter:image" , content:this.katSeo.kapak },
    ...
    async asyncData({ $axios, route }){
        const data = await $axios.get('/kategoriler').then(resp => { return resp.data } );
          let katSeo = await $axios.get('/static-seo?pos=Kategoriler').then(resp => {
             return resp.data
         }) 
        return {
            data, katSeo
